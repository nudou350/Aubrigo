...
describe("UsersService", () => {
  ...
  describe("changePassword", () => {
    ...
    it("should throw BadRequestException when passwords do not match", async () => {
      ...
      await expect(
        service.changePassword(mockUser.id as string, invalidDto),
  ...
  describe("findAll", () => {
    ...
    beforeEach(() => {
      ...
      mockUserRepository.manager.createQueryBuilder.mockReturnValue(
    ...
    it("should filter by search (ONG name)", async () => {
      const mockOngs = [
        { id: "1", ongName: "Animal Shelter", location: "Lisbon" },
      ];
      mockQueryBuilder.getMany.mockResolvedValue(mockOngs);
      mockManagerQueryBuilder.getRawOne.mockResolvedValue(null);
  ...
  describe("findOngById", () => {
    ...
    it("should throw NotFoundException when ONG not found", async () => {
      mockUserRepository.findOne.mockResolvedValue(null);
      await expect(service.findOngById("non-existent-id")).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.findOngById("non-existent-id")).rejects.toThrow(
        "ONG not found",
    ...
    it("should only return approved ONGs", async () => {
      mockUserRepository.findOne.mockResolvedValue(null);
      await expect(service.findOngById("pending-ong-id")).rejects.toThrow(
        NotFoundException,
      );
      expect(mockUserRepository.findOne).toHaveBeenCalledWith({
