...
describe("PetsService", () => {
  ...
  const mockCacheService = {
    ...
    set: jest.fn(),
    delete: jest.fn(),
    getPetsCacheKey: jest.fn().mockReturnValue("pets:cache:key"),
  };
  beforeEach(async () => {
    ...
    petImageRepository = module.get<Repository<PetImage>>(
      getRepositoryToken(PetImage),
    );
    jest.clearAllMocks();
  });
  ...
  describe("create", () => {
    ...
    it("should create pet successfully", async () => {
      mockPetRepository.manager.findOne.mockResolvedValue({
        id: mockOngId,
        countryCode: "PT",
      });
  ...
  describe("update", () => {
    it("should update own pet successfully", async () => {
      ...
      mockPetRepository.findOne.mockResolvedValueOnce({
        ...mockPet,
        name: "Updated Max",
      });
    ...
    it("should throw ForbiddenException when updating other ONG pet", async () => {
      mockPetRepository.findOne.mockResolvedValue(mockPet);
      await expect(
        service.update("pet-123", {}, "different-ong", []),
