...
export class CountryService {
  ...
  async detectCountryFromRequest(req: any): Promise<string> {
    ...
    if (customCountry) {
      console.log(
        "[CountryService] Using custom header country:",
        customCountry,
      );
    ...
    if (cloudFrontCountry) {
      console.log(
        "[CountryService] Using CloudFront country:",
        cloudFrontCountry,
      );
    ...
    if (cloudflareCountry) {
      console.log(
        "[CountryService] Using Cloudflare country:",
        cloudflareCountry,
      );
    ...
    if (
      ...
      try {
        ...
        if (detectedCountry) {
          console.log(
            `[CountryService] ✅ Detected country ${detectedCountry} from IP ${clientIp}`,
          );
      ...
      console.log(
        "[CountryService] ⚠️ Local/private IP detected, cannot determine country from IP",
      );
    }

    // Priority 5: Default to PT
    console.log(
      "[CountryService] No detection method available, defaulting to PT",
    );
    return "PT";
  }
