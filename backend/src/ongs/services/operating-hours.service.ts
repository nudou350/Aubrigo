...
export class OperatingHoursService {
  ...
  async create(
    ...
    const normalizedDto = {
      ...
      lunchBreakStart: dto.lunchBreakStart
        ? this.normalizeTime(dto.lunchBreakStart)
        : undefined,
      lunchBreakEnd: dto.lunchBreakEnd
        ? this.normalizeTime(dto.lunchBreakEnd)
        : undefined,
    };
    // Validate time format
    this.validateTimeFormat(normalizedDto.openTime);
    this.validateTimeFormat(normalizedDto.closeTime);
    if (normalizedDto.lunchBreakStart)
      this.validateTimeFormat(normalizedDto.lunchBreakStart);
    if (normalizedDto.lunchBreakEnd)
      this.validateTimeFormat(normalizedDto.lunchBreakEnd);
    // Validate time logic
    if (normalizedDto.isOpen) {
      this.validateTimeOrder(
        normalizedDto.openTime,
        normalizedDto.closeTime,
        "Open time must be before close time",
      );
      if (normalizedDto.lunchBreakStart && normalizedDto.lunchBreakEnd) {
        this.validateTimeOrder(
          normalizedDto.lunchBreakStart,
          normalizedDto.lunchBreakEnd,
          "Lunch break start must be before end",
        );
        this.validateTimeOrder(
          normalizedDto.openTime,
          normalizedDto.lunchBreakStart,
          "Lunch break must be within operating hours",
        );
        this.validateTimeOrder(
          normalizedDto.lunchBreakEnd,
          normalizedDto.closeTime,
  ...
  private async createDefaultOperatingHours(
    ongId: string,
  ): Promise<OngOperatingHours[]> {
    const defaultHours = [
  ...
  async findByOngAndDay(
    ongId: string,
    dayOfWeek: number,
  ): Promise<OngOperatingHours | null> {
    ...
    if (!hours) {
      const allHours = await this.operatingHoursRepository.find({
        where: { ongId },
      });
  ...
  async update(
    ...
    if (!existing) {
      throw new NotFoundException(
        `Operating hours for day ${dayOfWeek} not found`,
      );
    }
    ...
    if (merged.isOpen) {
      ...
      if (merged.lunchBreakStart && merged.lunchBreakEnd) {
        this.validateTimeOrder(
          merged.lunchBreakStart,
          merged.lunchBreakEnd,
          "Lunch break start must be before end",
        );
        this.validateTimeOrder(
          merged.openTime,
  ...
  async delete(ongId: string, dayOfWeek: number): Promise<void> {
    const result = await this.operatingHoursRepository.delete({
      ongId,
      dayOfWeek,
    });
  ...
  private validateTimeFormat(time: string): void {
    ...
    if (!timeRegex.test(time)) {
      throw new BadRequestException(
        `Invalid time format: ${time}. Expected format: HH:mm (e.g., 09:00)`,
      );
    }
  }
