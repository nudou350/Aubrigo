...
export class AddAppointmentSchedulingSystem1736100000000
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Create ong_operating_hours table
    const operatingHoursTableExists = await queryRunner.hasTable(
      "ong_operating_hours",
    );
    ...
    const appointmentSettingsTableExists = await queryRunner.hasTable(
    ...
    const exceptionsTableExists = await queryRunner.hasTable(
    ...
    if (
      appointmentsTable &&
      !appointmentsTable.findColumnByName("scheduled_start_time")
    ) {
    ...
    if (
      appointmentsTable &&
      !appointmentsTable.findColumnByName("scheduled_end_time")
    ) {
      await queryRunner.addColumn(
        "appointments",
        new TableColumn({
