...
describe("AppointmentsService", () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: AvailableSlotsService, useValue: mockAvailableSlotsService },
        {
          ...
          useValue: mockAppointmentSettingsService,
        },
      ],
    }).compile();
  ...
  describe("create", () => {
    it("should create appointment successfully (legacy system)", async () => {
      ...
      mockEmailService.sendAppointmentConfirmationToVisitor.mockResolvedValue(
        true,
      );
      ...
      expect(
        mockEmailService.sendAppointmentConfirmationToVisitor,
      ).toHaveBeenCalled();
      expect(
        mockEmailService.sendAppointmentNotificationToOng,
      ).toHaveBeenCalled();
    });
    it("should create appointment with scheduled time (new system)", async () => {
      ...
      mockEmailService.sendAppointmentAutoConfirmedToVisitor.mockResolvedValue(
        true,
      );
      mockEmailService.sendAppointmentAutoConfirmedToOng.mockResolvedValue(
        true,
      );
      const result = await service.create({
        petId: "pet-1",
      ...
      expect(result).toBeDefined();
      expect(
        mockEmailService.sendAppointmentAutoConfirmedToVisitor,
      ).toHaveBeenCalled();
    });
  ...
  describe("cancel", () => {
    it("should cancel appointment", async () => {
      ...
      mockEmailService.sendAppointmentCancellationToVisitor.mockResolvedValue(
        true,
      );
